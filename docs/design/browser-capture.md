...
> **Status: built, withdrawn from the UI in 0.5.0, and removed from the
> code afterwards.** It worked on pages that sit still and failed on
> anything that redraws itself between grabs: the stitcher located joins
> by runs of pixel-identical rows, and a live chart or a playing video
> breaks every run. Coming back to this means a tolerant match -- accept
> an overlap when most rows line up -- and git history has the first
> attempt to start from. Everything below describes the intended feature.
