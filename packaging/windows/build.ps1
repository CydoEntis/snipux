# Builds a standalone snipux.exe (SNX-96) that runs on a Windows machine
# with no Python installed. Run from a checkout with Python 3.10+ and pip
# on PATH:
#
#   powershell -File packaging\windows\build.ps1
#
# Produces dist\snipux.exe. Every path below is resolved relative to this
# script, not the caller's working directory, so it can be run from
# anywhere.
